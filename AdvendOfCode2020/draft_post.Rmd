...
# generate all combination of 2 and generate a dataframe
comb <- as.data.frame(t(combn(input, 2)))

# sum each combinations
rSums <- rowSums(comb)

# find which one has the sum equals 2020
vals <- comb[rSums==2020,]

# multiply them
resp <- prod(unlist(vals))
...
findCombSum <- function(in.data, n.comb, match.value){
  ...
  # generate all combination of ''n.comb' and generate a dataframe
  comb <- as.data.frame(t(combn(in.data, n.comb)))
  
  # sum each combinations
  rSums <- rowSums(comb)
  
  # find which one has the sum equals 'match.value'
  vals <- comb[rSums==match.value,]
