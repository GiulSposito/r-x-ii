...
```{r}
# separete and str_count come in handy for this
library(tidyverse)
...
password.check <- read.csv("./day02_input.txt", header = F) %>% 
  ...
  separate(policy, c("policy.range","policy.letter"), sep=" ") %>%
  separate(policy.range, c("min","max"), sep = "-", convert = T) %>% 
  mutate( count.letter = str_count(password, policy.letter)) %>% 
  mutate( valid.password = count.letter >= min & count.letter <= max )

# how many password are valid?
sum(password.check$valid.password)
